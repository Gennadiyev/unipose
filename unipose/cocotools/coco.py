...
class COCO:
    """UniPose modified the C{pycocotools/coco.py} file to use the faster orjson library and added support to discard all unlabelled images."""
    ...
    def createIndex(self):
        ...
        # print('index created!')
        # if an image has no annotations, it will not be added to the index
        imgIds = list(imgs.keys())
        for img in imgIds:
            _anns = imgToAnns[img]
            if len(_anns) == 0:
                imgs.pop(img)
    ...
    def download(self, tarDir = None, imgIds = [] ):
        ...
        '''
        if tarDir is None:
            raise ValueError('Please specify target directory')
            return -1
        if len(imgIds) == 0:
