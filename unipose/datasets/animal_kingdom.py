...
class AnimalKingdomDataset(BaseJointDataset):
    ...
    def __init__(
        ...
        sub_category: Literal[
            "ak_P1", "ak_P2", "ak_P3_amphibian", "ak_P3_bird", "ak_P3_fish", "ak_P3_mammal", "ak_P3_reptile"
        ] = "ak_P1",
        ...
        if sub_category not in [
            ...
            raise ValueError(
                "Invalid sub-category {} (must be one of 'ak_P1', 'ak_P2', 'ak_P3_amphibian', 'ak_P3_bird', 'ak_P3_fish', 'ak_P3_mammal', 'ak_P3_reptile')".format(
                    sub_category
                )
            )
        ...
        self.split = split
        self.json = os.path.join(path, "annotation_coco", self.sub_category, f"{self.split}.json")
    ...
    def __getitem__(self, idx):
        ...
        image = cv2.imread(os.path.join(self.path, "dataset", image_path))
